...
class AccountMove(models.Model):
    ...
    def get_bank_balance(self):
        ...
        self._cr.execute("""select account_account.name as name, sum(balance) as balance,
                            min(account_account.id) as id, max(company.name) as company from account_move_line left join
                            account_account on account_account.id = account_move_line.account_id 
                            left join res_company company on account_account.company_id = company.id
                            where
